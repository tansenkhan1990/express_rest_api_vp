...
export const refresh = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // Token rotation: issue new tokens first, then revoke the old one.
    // This prevents lock-out if generateRefreshToken fails.
...
export const logout = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (refreshToken) {
      // Revoke the refresh token — user can no longer refresh their access
      await revokeRefreshToken(refreshToken);
    }
